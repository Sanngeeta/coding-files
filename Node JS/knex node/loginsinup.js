...
function insert_data(){
    knex.schema.hasTable('users').then((tableExits)=>{
        if(tableExits){
            const array=[]
            const k=require('readline-sync')
            const num=k.questionInt('Enter the number of data: ')
            for(i=0;i<num;i++){
                const user=require('readline-sync')
                const id=user.question('Enter the ID: ')
                const name=user.question('Enter the name: ')
                const lastname=user.question('Enter the last name :')

            dict={id:id,first_name:name,last_name:lastname}
            array.push(dict)

            }
            knex('users').insert(array).then(()=>{
...
// // let num=JSON.parse(k)
// // fs.readFileSync("dic.json")
// // console.log(num)
